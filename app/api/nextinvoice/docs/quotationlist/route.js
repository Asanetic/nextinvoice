...
import {QuotationlistRowMutations} from './QuotationlistRowMutations';
...
export async function GET(request) {
  try {
    ...
    const mutationsObj = isEmpty(requestedMutationsObj) ? listQuotationlistRowMutationsKeys : requestedMutationsObj;
    
    if(requestValid){
    
      const result = await mosyFlexSelect(enhancedParams, mutationsObj, QuotationlistRowMutations);

      return Response.json({
        status: 'success',
        message: 'Quotationlist data retrieved',
        ...result,
      });
      
   }
  } catch (err) {
    console.error('GET Quotationlist failed:', err);
    return Response.json(
...
export async function POST(QuotationlistRequest) {
  try {
    let body;
    let isMultipart = false;

    const contentType = QuotationlistRequest.headers.get("content-type") || "";

    if (contentType.includes("multipart/form-data")) {
      isMultipart = true;
      const formData = await QuotationlistRequest.formData();
      ...
      body = await QuotationlistRequest.json();
    }
    
    
    const { valid: isTokenValid, reason: tokenError, data: authData } = processAuthToken(QuotationlistRequest);
  ...
  //--- End invoices inputs array --//
    ...
    return Response.json({
      status: 'error',
      message: `Invalid action: ${QuotationlistFormAction}`
    }, { status: 400 });
