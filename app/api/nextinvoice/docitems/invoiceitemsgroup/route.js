...
import {InvoiceitemsRowMutations} from '../invoiceitems/InvoiceitemsRowMutations';

import listInvoiceitemsRowMutationsKeys from '../invoiceitems/InvoiceitemsMutationKeys';
...
export async function GET(request) {
  try {
    ...
    const groupQuery = `${base64Decode(queryParams.q)} group by item_name, rate`
    const selColStr = base64Encode(`item_name, CONCAT( item_name,  ' @ ' , rate) as item_summary, rate , item_remark`);

    // ✅ Provide default fallbacks
    const enhancedParams = {
      tbl: 'invoice_items',
      colstr:  selColStr, // default to *
      ...queryParams ,
      q:base64Encode(groupQuery)
    };
