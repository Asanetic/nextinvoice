...
import { DeleteBusinesslist } from '../businesslist/BusinesslistDbGateway';

export async function GET(request) {

  const { searchParams } = new URL(request.url);

  const deleteToken = searchParams.get('_companies_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // 👇 Customize table and WHERE clause here
    const table = 'companies'; // Replace with your actual table
    
     
          
          const deleteAttachedMedia = await mosyQddata('companies', 'primkey', deleteTokenDecode);
    ...
    const res = await DeleteBusinesslist(deleteTokenDecode, whereStr);
